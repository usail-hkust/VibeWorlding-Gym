...
def apply_patch_mbridge():
    ...
    # Patch mbridge's split_deepstack_embs to handle None deepstack_visual_embeds.
    # When a micro-batch has no images (or vision encoder returns None for deepstack),
    # the original function crashes with "TypeError: 'NoneType' object is not iterable".
    try:
        import mbridge.models.qwen3_vl.utils as _mbridge_qwen3_vl_utils
        import mbridge.models.qwen3_vl.model as _mbridge_qwen3_vl_model

        _orig_split_deepstack_embs = _mbridge_qwen3_vl_utils.split_deepstack_embs

        def _patched_split_deepstack_embs(*args, **kwargs):
            """Wrapper that guards against None deepstack_visual_embeds."""
            # The function signature is split_deepstack_embs(visual_pos_masks, deepstack_visual_embeds, ...)
            # or split_deepstack_embs(visual_pos_masks=..., deepstack_visual_embeds=...)
            # We need to check if deepstack_visual_embeds is None
            deepstack_arg = None
            if len(args) >= 2:
                deepstack_arg = args[1]
            elif "deepstack_visual_embeds" in kwargs:
                deepstack_arg = kwargs["deepstack_visual_embeds"]

            if deepstack_arg is None:
                # Return empty results: (None, None) or (visual_pos_masks, [])
                # to match what callers expect
                visual_pos_masks = args[0] if len(args) >= 1 else kwargs.get("visual_pos_masks", None)
                return visual_pos_masks, []

            return _orig_split_deepstack_embs(*args, **kwargs)

        _mbridge_qwen3_vl_utils.split_deepstack_embs = _patched_split_deepstack_embs
        # Also patch the reference in the model module if it imported the function directly
        if hasattr(_mbridge_qwen3_vl_model, "split_deepstack_embs"):
            _mbridge_qwen3_vl_model.split_deepstack_embs = _patched_split_deepstack_embs
    except (ImportError, AttributeError):
        # mbridge qwen3_vl module not available, skip patching
        pass
