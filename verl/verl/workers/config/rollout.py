...
class MultiTurnConfig(BaseConfig):
    ...
    # MapGenAgentLoop 专用配置
    auto_render: Optional[bool] = None
    pcg_server_url: Optional[str] = None
    pcg_script_path: Optional[str] = None
    enable_verify: Optional[bool] = None
    verify_model: Optional[str] = None
    log_dir: Optional[str] = None
    per_turn_max_tokens: Optional[int] = None
    pcg_max_concurrency: int = 16
    # PCG 渲染模式: "gradio" (新 Gradio 服务) | "legacy" (老 subprocess + HTTP 服务)
    pcg_mode: str = "gradio"
    pcg_gradio_server: str = os.environ.get("PCG_GRADIO_SERVER", "http://localhost:8080")
    # Anti-Hacking: 效率折扣参数
    reward_efficiency_alpha: float = 0.0   # 按首次做对轮次折扣 (0=禁用)
    reward_efficiency_beta: float = 0.0    # 多余轮次惩罚 (0=禁用)
    max_turns_by_type: Optional[str] = None  # 动态max_turns "type1:3,type3:5"
    # verified 严格作用域校验: criteria 未授权的越界改动(多删/误删/多加/擅移)即清零 reward。
    # 1=开启(默认) / 0=关闭。shell 传入 int，agent_loop 侧按字符串宽松解析。
    verified_strict_scope: int = 1
    retrieve_url: str = os.environ.get("RETRIEVE_SERVER_URL", "http://localhost:8081")  # 资产检索服务地址（generate 任务专用）
    pcg_item_infos_path: str = ""  # PCG item_infos 白名单路径，空串则走内置默认路径
