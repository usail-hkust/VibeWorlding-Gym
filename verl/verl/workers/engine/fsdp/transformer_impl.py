...
class FSDPEngineWithLMHead(FSDPEngine):
        ...
        if use_remove_padding:
            ...
            if pad_mode == DatasetPadMode.NO_PADDING:
                ...
                if position_ids.dim() == 3:
                    # Debug: check ragged_idx and shapes before .values()
                    _ragged_idx = getattr(position_ids, "_ragged_idx", "N/A")
                    _pos_values = position_ids.values()
                    print(f"[DEBUG ulysses] position_ids.dim()=3, _ragged_idx={_ragged_idx}, "
                          f"values.shape={_pos_values.shape}, input_ids_rmpad.shape={input_ids_rmpad.shape}",
                          flush=True)
                    position_ids_rmpad = _pos_values.unsqueeze(1)  # (4, 1, total_nnz)
                else:
    ...
    def forward_step(self, micro_batch: TensorDict, loss_function, forward_only):
        ...
        # Fix 3D position_ids ragged index that may be lost during micro_batch splitting
        tu.maybe_fix_3d_position_ids(micro_batch)
