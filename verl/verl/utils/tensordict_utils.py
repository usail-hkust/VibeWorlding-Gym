...
def concat_nested_tensors(tensors: list[torch.Tensor]) -> torch.Tensor:
    ...
    for tensor in tensors:
        ...
        try:
            ...
            unbind_tensor = [padded[j, :length] for j, length in enumerate(lengths)]
    ...
    if batch is not None:
        for key, tensor in batch.items():
            if isinstance(tensor, torch.Tensor) and not tensor.is_nested:
                ...
                # Workaround for PyTorch bug with 3D+ jagged NestedTensors
                # (e.g. Qwen-VL mRoPE position_ids with shape (4, seq_len) per sample).
                # See https://github.com/pytorch/pytorch/issues/153238
                orig_ragged_idx = getattr(tensor, "_ragged_idx", 1)
                try:
                    tensor_lst = tensor.unbind()  # fast path for 2D jagged
                except RuntimeError:
                    padded = tensor.to_padded_tensor(0)
                    offsets = tensor.offsets()
                    lengths = offsets.diff().tolist()
                    tensor_lst = [padded[j, :length] for j, length in enumerate(lengths)]
                new_nested = torch.nested.as_nested_tensor(
                    [tensor_lst[idx] for idx in indices], layout=torch.jagged
                )
                # Preserve the original ragged index for 3D+ nested tensors (e.g. VLM position_ids)
                if orig_ragged_idx != 1:
                    new_nested._ragged_idx = orig_ragged_idx
                data_dict[key] = new_nested
            else:
