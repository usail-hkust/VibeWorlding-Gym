...
class SFTTensorCollator:
    ...
    def collate_variable_batch(self, batch: list[dict[str, any]]) -> dict[str, any]:
        ...
        # multi_modal_inputs is a per-sample dict of tensors (or absent for text-only samples).
        # Collect it as a list[dict|None] so get_tensordict can store it as NonTensorData,
        # consistent with how verl workers (dp_actor, dp_critic) expect it.
        if "multi_modal_inputs" in tensor_keys:
            tensor_keys.discard("multi_modal_inputs")
            final_batch["multi_modal_inputs"] = [item.get("multi_modal_inputs", None) for item in batch]
