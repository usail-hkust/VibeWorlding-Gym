...
class MultiTurnSFTDataset(Dataset):
    ...
    def _read_files_and_process(self):
        ...
        for parquet_file in self.parquet_files:
            ...
            # NOTE: 原先用 pd.read_parquet(..., dtype_backend="pyarrow")，在数据
            # 同时包含 (1) 大 bytes 列(如图片) 被自动切成多 data page 和 (2) 深
            # 嵌套 list<struct> 列(如 tools schema) 时，会触发 pyarrow 的
            # ArrowNotImplementedError: Nested data conversions not implemented
            # for chunked array outputs。
            # 这里按小 batch 读取，每个 batch 内部每列都是单 chunk（规避上述 bug），
            # 然后**在 pandas 层 concat**（而不是在 pyarrow 层 combine_chunks，
            # 后者在大 bytes 列总量 > 2GB 时会 offset overflow，因为 Arrow binary
            # 用 int32 offsets 上限 2GB）。
            import pyarrow.parquet as pq
            pf = pq.ParquetFile(parquet_file)
            batch_dfs = []
            for batch in pf.iter_batches(batch_size=64):
                # 单 batch 内每列都是单 chunk，to_pandas 不会触发嵌套列 chunked bug
                batch_dfs.append(batch.to_pandas(types_mapper=pd.ArrowDtype))
            if batch_dfs:
                dataframe = pd.concat(batch_dfs, ignore_index=True)
            else:
    ...
    def _fix_multi_modal_after_truncation(
        self, input_ids: torch.Tensor, multi_modal_inputs: dict
    ) -> tuple[torch.Tensor, dict]:
        """Fix multi_modal_inputs after truncation to ensure image/video tokens match features.

        When input_ids is truncated (left or right), some image/video tokens may be partially
        removed. This method removes the corresponding pixel_values and grid_thw entries for
        images/videos whose tokens are no longer complete in the truncated input_ids.
        It also removes any orphaned partial image/video tokens from input_ids.

        Returns:
            Updated (input_ids, multi_modal_inputs) tuple.
        """
        ...
        merge_size = getattr(self.processor.image_processor, "merge_size", 2) if hasattr(self.processor, "image_processor") else 2

        # Fix images
        image_grid_thw = multi_modal_inputs.get("image_grid_thw", None)
        pixel_values = multi_modal_inputs.get("pixel_values", None)
        if image_token_id is not None and image_grid_thw is not None and pixel_values is not None:
            # Calculate expected tokens per image: t * (h // merge_size) * (w // merge_size)
            tokens_per_image = (
                image_grid_thw[:, 0]
                * (image_grid_thw[:, 1] // merge_size)
                * (image_grid_thw[:, 2] // merge_size)
            )
            # Calculate pixel patches per image: t * h * w
            patches_per_image = image_grid_thw.prod(dim=1)

            n_image_tokens_after_trunc = (input_ids == image_token_id).sum().item()
            total_expected_tokens = tokens_per_image.sum().item()

            if n_image_tokens_after_trunc < total_expected_tokens:
                # Find how many complete images fit in the truncated tokens
                cumsum_tokens = tokens_per_image.cumsum(dim=0)
                keep_images = (cumsum_tokens <= n_image_tokens_after_trunc).sum().item()

                if keep_images > 0:
                    multi_modal_inputs["image_grid_thw"] = image_grid_thw[:keep_images]
                    keep_patches = patches_per_image[:keep_images].sum().item()
                    multi_modal_inputs["pixel_values"] = pixel_values[:keep_patches]

                    # Remove orphaned partial image tokens from input_ids
                    kept_tokens = tokens_per_image[:keep_images].sum().item()
                    if kept_tokens < n_image_tokens_after_trunc:
                        # Replace orphaned image tokens with pad token
                        pad_token_id = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else 0
                        image_positions = (input_ids == image_token_id).nonzero(as_tuple=True)[0]
                        orphan_positions = image_positions[kept_tokens:]
                        input_ids[orphan_positions] = pad_token_id
                else:
                    # No complete images remain - remove all image data
                    del multi_modal_inputs["image_grid_thw"]
                    del multi_modal_inputs["pixel_values"]
                    # Replace all remaining image tokens with pad token
                    pad_token_id = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else 0
                    input_ids[input_ids == image_token_id] = pad_token_id

        # Fix videos (same logic)
        video_grid_thw = multi_modal_inputs.get("video_grid_thw", None)
        pixel_values_videos = multi_modal_inputs.get("pixel_values_videos", None)
        if video_token_id is not None and video_grid_thw is not None and pixel_values_videos is not None:
            tokens_per_video = (
                video_grid_thw[:, 0]
                * (video_grid_thw[:, 1] // merge_size)
                * (video_grid_thw[:, 2] // merge_size)
            )
            patches_per_video = video_grid_thw.prod(dim=1)

            n_video_tokens_after_trunc = (input_ids == video_token_id).sum().item()
            total_expected_video_tokens = tokens_per_video.sum().item()

            if n_video_tokens_after_trunc < total_expected_video_tokens:
                cumsum_tokens = tokens_per_video.cumsum(dim=0)
                keep_videos = (cumsum_tokens <= n_video_tokens_after_trunc).sum().item()

                if keep_videos > 0:
                    multi_modal_inputs["video_grid_thw"] = video_grid_thw[:keep_videos]
                    keep_patches = patches_per_video[:keep_videos].sum().item()
                    multi_modal_inputs["pixel_values_videos"] = pixel_values_videos[:keep_patches]

                    kept_tokens = tokens_per_video[:keep_videos].sum().item()
                    if kept_tokens < n_video_tokens_after_trunc:
                        pad_token_id = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else 0
                        video_positions = (input_ids == video_token_id).nonzero(as_tuple=True)[0]
                        orphan_positions = video_positions[kept_tokens:]
                        input_ids[orphan_positions] = pad_token_id
                else:
                    del multi_modal_inputs["video_grid_thw"]
                    del multi_modal_inputs["pixel_values_videos"]
                    pad_token_id = self.tokenizer.pad_token_id if self.tokenizer.pad_token_id is not None else 0
                    input_ids[input_ids == video_token_id] = pad_token_id
    ...
    def __getitem__(self, item):
        ...
        if self.pad_mode == DatasetPadMode.RIGHT:
            ...
            # Fix multi_modal_inputs after truncation to keep image/video tokens consistent
            if truncated and len(multi_modal_inputs) > 0:
                input_ids, multi_modal_inputs = self._fix_multi_modal_after_truncation(
                    input_ids, multi_modal_inputs
                )
            ...
            # Debug: verify input_ids and position_ids have consistent lengths
            _ids_len = input_ids.shape[0]
            _pos_len = position_ids.shape[-1]
            assert _ids_len == _pos_len, (
                f"[DEBUG dataset] input_ids.shape={input_ids.shape}, position_ids.shape={position_ids.shape}, "
                f"truncated={truncated}, sequence_length={sequence_length}, max_length={self.max_length}"
            )
            res = {
                "input_ids": input_ids,
                "position_ids": position_ids,
