...
# 训练 hang 时的定位手段：对某个可疑 python 进程跑
#     kill -USR1 <pid>
# 它会把**所有线程**（包括 asyncio event loop、Ray worker、GC thread 等）的
# python 栈打印到 stderr（nohup 环境下就是 rl.log）。比 py-spy 更可靠，因为
# 是进程自己在栈顶中断，完全不依赖 ptrace/外挂进程。
#
# 只在每个 python 进程被 import 时注册一次；Ray 会给每个 worker 自动触发
# import，所以主进程 + 每个 actor worker 都会被覆盖。
try:
...
except Exception as _fh_err:  # noqa: F841
    # 某些受限环境（PID namespace 禁用 signal register）可能失败，不致命
    pass
