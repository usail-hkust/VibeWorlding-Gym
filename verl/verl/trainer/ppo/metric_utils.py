...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    # Handle case where all samples are aborted (e.g., all bad samples with reward=-100)
    ...
    if use_critic:
        ...
        return_diff_var = torch.var(valid_returns - valid_values) if valid_returns.numel() > 1 else torch.tensor(0.0)
        return_var = torch.var(valid_returns) if valid_returns.numel() > 1 else torch.tensor(1e-5)
    ...
    if non_aborted_response_length.numel() > 0:
        ...
        # All samples are aborted (e.g., all bad samples), use safe defaults
    ...
    # Safe handling for advantage/returns when all masked out
    ...
    if valid_returns.numel() > 0:
        ...
        returns_mean = returns_max = returns_min = 0.0
    ...
    # ===== Map Gen 核心监控指标（3 + 1） =====
    # 这些指标通过 MapGenAgentLoop 的 extra_fields 传递到 non_tensor_batch
    #
    # a. verifier_success_rate: reward 非 -100 的比例，衡量 verifier 稳定性
    # b. instruction_following_rate: 模型输出了 tool_call 时解析成功的比例，衡量格式遵循能力
    #    （None 值表示模型未尝试输出 tool_call，不参与统计）
    # c. pcg_sandbox_success_rate: PCG 沙盒调用成功率
    #    （None 值表示未调用 PCG，不参与统计）
    # d. interaction_turns: 交互轮次
    _map_gen_metrics = [
        ("map_gen_verifier_call_success_rate",  "map_gen/verifier_call_success_rate"),
        ("map_gen_verifier_parse_success_rate", "map_gen/verifier_parse_success_rate"),
        ("map_gen_instruction_following_rate",  "map_gen/instruction_following_rate"),
        ("map_gen_pcg_sandbox_success_rate",    "map_gen/pcg_sandbox_success_rate"),
        ("map_gen_pcg_convert_success_rate",    "map_gen/pcg_convert_success_rate"),   # 地图格式→actors 解析成功率
        ("map_gen_pcg_render_success_rate",     "map_gen/pcg_render_success_rate"),    # 渲染服务调用成功率
        ("map_gen_interaction_turns",           "map_gen/interaction_turns"),
        ("map_gen_bad_sample",                  "map_gen/bad_sample_rate"),
    ]
    for ntb_key, metric_key in _map_gen_metrics:
        if ntb_key in batch.non_tensor_batch:
            vals = batch.non_tensor_batch[ntb_key]
            try:
                # 过滤 None 值（None 表示该指标不适用于此样本）
                valid_vals = [float(v) for v in vals if v is not None]
                if valid_vals:
                    vals_arr = np.asarray(valid_vals, dtype=np.float32)
                    metrics[f"{metric_key}/mean"] = float(np.mean(vals_arr))
                    metrics[f"{metric_key}/min"] = float(np.min(vals_arr))
                    metrics[f"{metric_key}/max"] = float(np.max(vals_arr))
            except Exception:
                pass

    # ===== Map Gen Reward 分量（total / hard；soft 已移除） =====
    # 这些值通过 extra_fields 传入 non_tensor_batch，用于在 wandb 中展示 reward 组成
    for reward_key in ["total_reward", "hard_reward"]:
        if reward_key in batch.non_tensor_batch:
            vals = batch.non_tensor_batch[reward_key]
            try:
                valid_vals = [float(v) for v in vals if v is not None]
                if valid_vals:
                    vals_arr = np.asarray(valid_vals, dtype=np.float32)
                    metrics[f"map_gen/{reward_key}/mean"] = float(np.mean(vals_arr))
                    metrics[f"map_gen/{reward_key}/min"] = float(np.min(vals_arr))
                    metrics[f"map_gen/{reward_key}/max"] = float(np.max(vals_arr))
            except Exception:
                pass

    # ===== Map Gen Bad Sample 原因分布 =====
    # 按 bad_sample_reason 分组统计，便于定位哪类问题最严重
    try:
        bad_reasons = batch.non_tensor_batch.get("map_gen_bad_sample_reason", [])
        if bad_reasons is not None and len(bad_reasons) > 0:
            ...
            for r in bad_reasons:
                if r:  # 非空字符串才计入
                    ...
                    total_with_reason += 1
            n_samples = len(bad_reasons)
            for reason, cnt in reason_counts.items():
                metrics[f"map_gen/bad_sample_reason/{reason}"] = cnt / n_samples
    except Exception:
        pass

    # ===== Map Gen 增强监控：按 verifier_type / query_tag 分组 =====
    # 从 extra_fields 传递的 verify_result 中提取细粒度指标
    try:
        from collections import defaultdict

        # 收集分组数据
        verifier_types = batch.non_tensor_batch.get("verifier_type", [])
        query_tags = batch.non_tensor_batch.get("query_tag", [])
        verify_results = batch.non_tensor_batch.get("verify_result", [])

        if verifier_types is not None and len(verifier_types) > 0:
            # 按 verifier_type 分组 reward
            type_rewards = defaultdict(list)
            tag_rewards = defaultdict(list)
            # unverified 细粒度
            uv_hard_pass_list = []
            uv_h_scores = {"H1": [], "H2": [], "H3": [], "H4": []}
            uv_h3_sub_scores = {"H3_VU": [], "H3_VR": []}  # v2 verifier: 0-5 Likert scores

            for i, vt in enumerate(verifier_types):
                if vt is None:
                    continue
                vt = str(vt)

                # 获取对应的 verify_result
                vr = verify_results[i] if i < len(verify_results) and verify_results[i] else {}
                ...
                total_reward = vr.get("total_reward", 0.0)
                if total_reward is not None:
                    try:
                        type_rewards[vt].append(float(total_reward))
                    except (ValueError, TypeError):
                        pass

                # 按 query_tag 分组
                qt = query_tags[i] if i < len(query_tags) and query_tags[i] else ""
                qt = str(qt)
                if qt and total_reward is not None:
                    try:
                        tag_rewards[qt].append(float(total_reward))
                    except (ValueError, TypeError):
                        pass

                # unverified 细粒度
                if vt == "unverified":
                    hard_pass = vr.get("hard_pass", False)
                    uv_hard_pass_list.append(1.0 if hard_pass else 0.0)

                    hr = vr.get("hard_result", {})
                    for hk in ["H1", "H2", "H3", "H4"]:
                        h_val = hr.get(hk, {}).get("pass")
                        if h_val is not None:
                            uv_h_scores[hk].append(float(h_val))

                    # v2 verifier: H3 子维度 (score 0-5)
                    for h3k in ["H3_VU", "H3_VR"]:
                        h3_val = hr.get(h3k, {}).get("score")
                        if h3_val is not None:
                            uv_h3_sub_scores[h3k].append(float(h3_val))

            # 写入按 verifier_type 分组的 reward
            for vt, rewards in type_rewards.items():
                if rewards:
                    arr = np.asarray(rewards, dtype=np.float32)
                    metrics[f"map_gen/reward_by_type/{vt}/mean"] = float(np.mean(arr))
                    metrics[f"map_gen/reward_by_type/{vt}/count"] = len(rewards)

            # 写入按 query_tag 分组的 reward
            for qt, rewards in tag_rewards.items():
                if rewards:
                    arr = np.asarray(rewards, dtype=np.float32)
                    metrics[f"map_gen/reward_by_tag/{qt}/mean"] = float(np.mean(arr))

            # unverified 细粒度指标
            if uv_hard_pass_list:
                arr = np.asarray(uv_hard_pass_list, dtype=np.float32)
                metrics["map_gen/unverified_hard_pass_rate"] = float(np.mean(arr))

            for hk, vals in uv_h_scores.items():
                if vals:
                    metrics[f"map_gen/unverified_{hk}_pass_rate"] = float(np.mean(vals))

            # v2 verifier: H3 子维度 avg_score 和 ≥4 通过率
            for h3k, vals in uv_h3_sub_scores.items():
                if vals:
                    arr = np.asarray(vals, dtype=np.float32)
                    metrics[f"map_gen/unverified_{h3k}_avg_score"] = float(np.mean(arr))
                    metrics[f"map_gen/unverified_{h3k}_ge4_rate"] = float(np.mean(arr >= 4))
