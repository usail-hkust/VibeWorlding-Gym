...
def compute_grpo_outcome_advantage(
    ...
    # 检测 bad samples (reward <= -99，即 reward=-100 的样本)
    bad_sample_mask = (scores <= -99.0)  # (bsz,) bool tensor
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        # 收集 group scores 时排除 bad samples，避免 -100 污染 mean/std
        for i in range(bsz):
            if not bad_sample_mask[i].item():
                id2score[index[i]].append(scores[i])
        # 对于全部都是 bad sample 的 group，补充一个零值避免空列表
        for i in range(bsz):
            if index[i] not in id2score:
                id2score[index[i]].append(torch.tensor(0.0, device=scores.device))
        for idx in id2score:
        ...
        for i in range(bsz):
            if bad_sample_mask[i].item():
                # bad sample 的 advantage 直接置零
                scores[i] = 0.0
...
def agg_loss(
    ...
    if loss_agg_mode == "token-mean":
        ...
        # Avoid divide-by-zero when the whole micro batch is aborted
        # (e.g., all samples are bad_sample with response_mask fully zero).
        # masked_sum is 0 in that case, so loss becomes 0 and contributes no gradient.
        batch_num_tokens = torch.clamp(batch_num_tokens, min=1.0)
