...
def _strip_markdown_json(raw: str) -> str:
    """去除 tool_call 内部可能存在的 ```json...``` markdown 代码块包裹，以及各种不规则格式"""
    import re as _re
    s = raw.strip()
    # 标准格式：```json\n...\n```
...
class HermesToolParser(ToolParser):
    ...
    增强：支持 <tool_call>```json\n[...]\n```</tool_call> 格式（SFT 数据中的 markdown 包裹）
    """
    ...
    async def extract_tool_calls(
        ...
        # 如果完全没有 <tool_call> 标签，尝试修复：有 </tool_call> 但没 <tool_call>
        if self.tool_call_start_token not in text and self.tool_call_end_token in text:
            # 尝试在 ```json 或 [{ 前面补 <tool_call>
            import re as _re
            # 找 </tool_call> 位置
            tc_end_idx = text.find(self.tool_call_end_token)
            # 往前找 ```json 或 [{
            json_block_idx = text.rfind("```json", 0, tc_end_idx)
            if json_block_idx == -1:
                json_block_idx = text.rfind("[{", 0, tc_end_idx)
            if json_block_idx == -1:
                json_block_idx = text.rfind("{", 0, tc_end_idx)
            if json_block_idx >= 0:
                text = text[:json_block_idx] + "<tool_call>" + text[json_block_idx:]
                logger.info("[HermesToolParser] 补全了缺失的 <tool_call> 开头标签")
        ...
        for match in matches:
            # 先去除 markdown 代码块包裹
            cleaned = _strip_markdown_json(match)
            if not cleaned:
                continue
            try:
                parsed = json.loads(cleaned)
                # 兼容：整体是 list（多个 tool call 的数组）
                ...
                logger.error(f"Failed to decode tool call: {e}, raw: {cleaned[:200]}")
...
@ToolParser.register("qwen3_vl")
class Qwen3XMLToolParser(ToolParser):
    """
    Tool parser for qwen3_coder/qwen3.5/qwen3_vl model.
    ...
    def _try_parse_json_tool_calls(self, text: str) -> tuple[str, list[FunctionCall]]:
        """Fallback: 当 XML 格式解析失败时，尝试从 <tool_call>...</tool_call> 中提取 JSON 格式的 tool call。
        ...
        - <tool_call>{"name": "xxx", "arguments": {...}}</tool_call>
        """
        tool_call_json_regex = regex.compile(r"<tool_call>(.*?)</tool_call>", regex.DOTALL)
        matches = tool_call_json_regex.findall(text)
        if not matches:
            return text, []
        ...
        for match in matches:
            ...
            try:
                ...
                logger.warning(f"[Qwen3XMLToolParser] JSON fallback parse failed: {e}, raw: {cleaned[:200]}")

        if function_calls:
            content = tool_call_json_regex.sub("", text)
            logger.info(f"[Qwen3XMLToolParser] JSON fallback succeeded, parsed {len(function_calls)} tool calls")
            return content, function_calls
    ...
    async def extract_tool_calls(
        ...
        # 修复：如果没有 <tool_call> 但有 </tool_call>，尝试补全开头标签
        if self.tool_call_start_token not in text and self.tool_call_end_token in text:
            tc_end_idx = text.find(self.tool_call_end_token)
            # 往前找 ```json 或 [{ 或 {"name"
            json_start_idx = text.rfind("```json", 0, tc_end_idx)
            if json_start_idx == -1:
                json_start_idx = text.rfind("[{", 0, tc_end_idx)
            if json_start_idx == -1:
                json_start_idx = text.rfind('{"name"', 0, tc_end_idx)
            if json_start_idx == -1:
                json_start_idx = text.rfind("{", 0, tc_end_idx)
            # 也尝试找 XML 格式的 <function=
            xml_start_idx = text.rfind("<function=", 0, tc_end_idx)
            # 取最早出现的位置
            candidates = [i for i in [json_start_idx, xml_start_idx] if i >= 0]
            if candidates:
                insert_idx = min(candidates)
                text = text[:insert_idx] + "<tool_call>" + text[insert_idx:]
                logger.info("[Qwen3XMLToolParser] 补全了缺失的 <tool_call> 开头标签")
        ...
        try:
            ...
            if len(function_calls) == 0:
                # XML 格式没找到 <function=...>，尝试 JSON fallback
                return self._try_parse_json_tool_calls(text)
            ...
            # XML 解析异常时也尝试 JSON fallback
            try:
                content, calls = self._try_parse_json_tool_calls(text)
                if calls:
